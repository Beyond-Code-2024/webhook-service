...
export class AuthService {
  ...
  async registerOrganisation(
    ...
    this.logger.log(
      `REGISTER_ORGANISATION: recieved create organisation request: ${registerOrganisationRequest.name}`,
    );
    ...
    this.logger.log(
      `REGISTER_ORGANISATION: created organisation: name: ${organisation.name} id: ${organisation._id}`,
    );
    return {
  ...
  async organisationLogin(
    ...
    this.logger.log(
      `ORGANISATION_LOGIN: login request from email: ${loginOrganisationRequest.email}`,
    );
    const organisation =
